 ...
 */
class Support_Agent_Edit_Admin_Page extends \WP_Ultimo\Admin_Pages\Edit_Admin_Page {
	 ...
	 * @since 2.0.0
	 *
	 * @param array $capabilities Registered Capabilities.
	 * @return array
	 */
	public function get_permission_fields($capabilities) {
		...
		$fields[$slug_all] = array(
			'type'            => 'toggle',
			'title'           => __('Toggle All', 'wp-ultimo-support-agents'),
			'desc'            => __('Check to allow this agent to perform all actions on the platform.', 'wp-ultimo-support-agents'),
	...
	public function get_dashboard_network_widgets($widgets) {
		...
		$fields['network_dashboard_widgets_header'] = array(
			'title'           => __('Dashboard Network Widgets', 'wp-ultimo-support-agents'),
			'desc'            => __('Enable and disable dashboard widgets for this agent.', 'wp-ultimo-support-agents'),
			'type'            => 'toggle',
			'wrapper_classes' => 'wu-w-full wu-bg-gray-100',
			'html_attr'       => array(
		...
		foreach ($widgets as $key => $title) {
			...
			$fields["network_dashboard_widgets[{$key}]"] = array(
				'type'            => 'toggle',
				'title'           => $title,
				'desc'            => __('Toggle to disable.', 'wp-ultimo-support-agents'),
				'tooltip'         => '',
				'wrapper_classes' => 'wu-w-1/2 wu-float-left',
				'html_attr'       => array(
	...
	public function page_loaded() {
		...
		$support_agent = $this->get_object();

		if ($support_agent && is_super_admin($support_agent->get_user_id())) {

			$actions = array(
				'activate' => array(
					'title' => __('Edit User &rarr;', 'wp-ultimo'),
					'url'   => add_query_arg(array(
						'user_id' => $support_agent->get_user_id(),
					), network_admin_url('user-edit.php')) . '#user_login',
				)
			);

			$message = __('This user is a <strong>Super Admin</strong>. This means that the permission controls below will not apply to them. Remove the super admin status from this user to be able to control their permissions here.', 'wp-ultimo-support-agents');

			WP_Ultimo()->notices->add($message, 'error', 'network-admin', false, $actions);
	...
	public function register_widgets() {
		...
		$this->add_fields_widget('at_a_glance', array(
			'title'                 => __('At a Glance', 'wp-ultimo-support-agents'),
			'position'              => 'normal',
			...
			'fields'                => array(
				'id'         => array(
					'type'          => 'text-display',
					'copy'          => true,
					'title'         => __('Support Agent ID', 'wp-ultimo-support-agents'),
					'display_value' => $this->get_object()->get_id(),
					'tooltip'       => '',
				),
				'last_login' => array(
					'edit'          => false,
					'title'         => __('Last Login', 'wp-ultimo-support-agents'),
					'type'          => 'text-edit',
					'display_value' => __('Never', 'wp-ultimo-support-agents'),
				),
			),
		));

		$capabilities = \WP_Ultimo_Support_Agents\Permission_Control::get_instance()->registered_capabilities();

		$capabilities_wordpress = \WP_Ultimo_Support_Agents\Permission_Control::get_instance()->get_capabilities_default_wordpress();

		$network_dashboard_widgets = \WP_Ultimo\Dashboard_Widgets::get_registered_dashboard_widgets();

		$this->add_tabs_widget('options', array(
			'title'    => __('Support Agent Options', 'wp-ultimo-support-agents'),
			'position' => 'normal',
			'sections' => array(
				'capabilities'              => array(
					'title'                 => __('WP Ultimo Permissions', 'wp-ultimo-support-agents'),
					...
					'desc'                  => __('Select what level of access this support access will have.', 'wp-ultimo-support-agents'),
				...
				'capabilities_wordpress'    => array(
					'title'                 => __('WordPress Permissions', 'wp-ultimo-support-agents'),
					'field_wrapper_classes' => 'wu-w-1/2 wu-box-border wu-items-center wu-flex wu-justify-between wu-p-4 wu-m-0 wu-border-t wu-border-l-0 wu-border-r-0 wu-border-b-0 wu-border-gray-300 wu-border-solid',
					'desc'                  => __('Select what level of access this support access will have.', 'wp-ultimo-support-agents'),
				...
				'network_dashboard_widgets' => array(
					'title'                 => __('Dashboard Widgets', 'wp-ultimo-support-agents'),
					'field_wrapper_classes' => 'wu-w-1/2 wu-box-border wu-items-center wu-flex wu-justify-between wu-p-4 wu-m-0 wu-border-t wu-border-l-0 wu-border-r-0 wu-border-b-0 wu-border-gray-300 wu-border-solid',
					'desc'                  => __('Select what level of access this support access will have.', 'wp-ultimo-support-agents'),
		...
		$this->add_save_widget('save', array(
			...
			'fields' => array(
				'user_id' => array(
					'type'              => 'model',
					'title'             => __('User', 'wp-ultimo-support-agents'),
	...
	public function get_title() {
		...
		return $this->edit ? __('Edit Support Agent', 'wp-ultimo-support-agents') : __('Add new Support Agent', 'wp-ultimo-support-agents');
	...
	public function get_menu_title() {
		...
		return __('Edit Support Agent', 'wp-ultimo-support-agents');
	...
	public function get_labels() {
		...
		return array(
			'edit_label'          => __('Edit Support Agent', 'wp-ultimo-support-agents'),
			'add_new_label'       => __('Add new Support Agent', 'wp-ultimo-support-agents'),
			'updated_message'     => __('Support Agent updated with success!', 'wp-ultimo-support-agents'),
			'title_placeholder'   => __('Enter Support Agent', 'wp-ultimo-support-agents'),
			'title_description'   => '',
			'save_button_label'   => __('Save Support Agent', 'wp-ultimo-support-agents'),
			'save_description'    => '',
			'delete_button_label' => __('Delete Support Agent', 'wp-ultimo-support-agents'),
			'delete_description'  => __('Be careful. This action is irreversible.', 'wp-ultimo-support-agents'),
		);
	 ...
	 *
	 * @since 2.0.0
	 * @return \WP_Ultimo_Support_Agents\Models\Support_Agent
	 */
	public function get_object() {
		...
		$item = wusa_get_support_agent($item_id);
